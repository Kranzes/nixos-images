{
  ...
  inputs = {
    ...
    hercules-ci-effects = { url = "github:Kranzes/hercules-ci-effects"; inputs.nixpkgs.follows = "nixos-unstable"; };
  };
          ...
          filesPerSystem = { pkgs, self', system, ... }: [
            ...
            }
            {
              ...
              path = pkgs.runCommand "sha256sums-${system}" { } ''
                ${pkgs.lib.concatMapStringsSep "\n"({ label, path }: if label == "sha256sums-${system}" then "true" else "ln -s ${path} ${label}") config.hercules-ci.github-releases.finalFiles}
                sha256sum * > "$out"
              '';
            }
          ];
